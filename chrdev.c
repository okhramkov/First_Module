...
static int mychrdev_open(struct inode *inode, struct file *file) {
    printk(KERN_INFO "opening devise %s:\n\n", MYDEV_NAME);
    return 0;
}
static int mychrdev_release(struct inode *inode, struct file *file) {
    printk(KERN_INFO "releasing devise %s:\n\n", MYDEV_NAME);
    return 0;
}
static ssize_t mychrdev_read(struct file *file, char __user *buf, size_t lbuf, loff_t *ppos) {
    ...
    *ppos += nbytes;
    printk(KERN_INFO "reading devise %s nbytes = %d, ppos = %d \n\n" , MYDEV_NAME, nbytes, (int) *ppos);
    return nbytes;
}
...
static ssize_t mychrdev_write(struct file *file, const char __user *buf, size_t lbuf, loff_t *ppos) {
    ...
    *ppos += nbytes;
    printk(KERN_INFO "writting devise %s nbytes = %d, ppos = %d \n\n" , MYDEV_NAME);
    return nbytes;
}
